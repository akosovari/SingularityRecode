...
public class GroundSpeed extends Actions {
    ...
    public void onMove(PlayerMoveEvent event) {
        ...
        if (!enabled || !Main.getInstance().enabled || data.deltaY > 0.49 || !data.clientGround || data.IcePosition || data.withElytra || data.usingRiptide || event.getPlayer().isInsideVehicle() || event.getPlayer().getAllowFlight() || abs(data.velXTicks) > 1 || abs(data.velZTicks) > 2 || data.SlimePosition) {
            return;
        }
        try {
            // Ground
