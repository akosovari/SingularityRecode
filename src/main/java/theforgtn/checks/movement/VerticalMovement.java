...
public class VerticalMovement extends Actions {
    ...
    public void onMove(PlayerMoveEvent event) {
        ...
        if (!enabled || data.withElytra || data.inCreative || player.getAllowFlight() || player.isInsideVehicle() || player.isInWater() || player.isGliding() || data.usingRiptide || data.clientGround) {
            return;
        }
        try {
            // STEP
